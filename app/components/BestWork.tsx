...
export default function BestWorks() {
  return (
    <section>
      <h2 className="mb-6 text-lg sm:text-xl font-medium">Best Works</h2>
      ...
      <div className="space-y-10 sm:space-y-16">
        {featured.map((project, index) => (
          <article
            key={index}
            className="group relative rounded-2xl p-px sm:p-[1.5px]"
            style={{
              background:
          ...
          >
            ...
            <div className="rounded-[12px] sm:rounded-[14px] overflow-hidden bg-[#0b0b0b] ring-1 ring-white/5 shadow-lg">
              {/* Browser bar (compact on mobile) */}
              <div className="flex items-center justify-between px-3 py-1.5 sm:px-4 sm:py-2 border-b border-white/10 bg-[#121212]">
                <div className="flex items-center gap-1.5 sm:gap-2">
                  <span className="h-2 w-2 sm:h-2.5 sm:w-2.5 rounded-full" style={{ background: "var(--c1, #ff52bf)" }} />
                  <span className="h-2 w-2 sm:h-2.5 sm:w-2.5 rounded-full" style={{ background: "var(--c2, #ffb900)" }} />
                  <span className="h-2 w-2 sm:h-2.5 sm:w-2.5 rounded-full" style={{ background: "var(--c3, #38bdf8)" }} />
                </div>
              ...
              {/* Screenshot: smaller on mobile, original on desktop */}
            ...
            <span
              aria-hidden
              className="hidden sm:block pointer-events-none absolute inset-0 rounded-2xl opacity-0 transition-opacity duration-300 group-hover:opacity-35"
              style={{
                filter: "blur(12px)",
                background:
